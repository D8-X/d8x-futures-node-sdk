...
import { MASK_MARKET_ORDER } from "../../test/tradeFunctionsAlt";
...
export default class AccountTrade extends PerpetualDataHandler {
    ...
    public static async _setAllowance(tokenAddr: string, proxyAddr: string, signer: ethers.Wallet, amount: BigNumber): Promise<string> {
    ...
    protected async _cancelOrder(symbol: string, orderId: string, orderBookContract: ethers.Contract | null): Promise<string | undefined> {
    ...
    private async _createSignature(
        ...
        const DOMAIN_TYPEHASH = ethers.utils.keccak256(Buffer.from("EIP712Domain(string name,uint256 chainId,address verifyingContract)"));
        let abiCoder = ethers.utils.defaultAbiCoder;
        let domainSeparator = ethers.utils.keccak256(
            abiCoder.encode(["bytes32", "bytes32", "uint256", "address"], [DOMAIN_TYPEHASH, ethers.utils.keccak256(Buffer.from(NAME)), chainId, proxyAddress])
        );
        ...
        let structHash = ethers.utils.keccak256(
            abiCoder.encode(
                ["bytes32", "uint24", "uint16", "address", "address", "int128", "int128", "int128", "uint256", "uint32", "int128", "uint256"],
                [
        ...
        let digest = ethers.utils.keccak256(abiCoder.encode(["bytes32", "bytes32", "bool"], [domainSeparator, structHash, isNewOrder]));
